...
class OpenFHEDemoBackend:
    """Encrypt, evaluate, decrypt inside one explicitly trusted demo request."""
    ...
    def evaluate(
        ...
        if needs_plaintext(operation):
            if values_b is None:
                raise ValueError("multiply_plain requires values_b")
            return he.decrypt(
                he.multiply_plain(left, values_b), len(values_a)
            )
