...
Inputs are non-empty numeric vectors. Vectors used together must have the same
length and belong to the same client session. CKKS uses approximate arithmetic,
so decrypted results can contain a small numerical error.
...
The script compares the HE results with ordinary Python calculations and prints
...
| `POST /v1/sessions/{id}/evaluate` | Operation and ciphertext IDs | Result ciphertext ID |
