...
| PyPI rejects the FIDES wheel as `linux_x86_64` | PyPI accepts portable manylinux platform tags rather than the builder's generic Linux tag. | Keep the `auditwheel repair --plat manylinux_2_35_x86_64` step in `gpu/Dockerfile`; do not publish the raw wheel. |
| Colab rejects or cannot load the FIDES wheel | The old wheel was built on Ubuntu 24.04 and required glibc 2.39. | Build release 0.3.4+ from the Ubuntu 22.04 CUDA stage and require the `manylinux_2_35_x86_64` wheel check in CI. |
| `auditwheel repair` reports `patchelf 0.14.3` but requires `>= 0.14.5` | Ubuntu 22.04's system `patchelf` is too old for current auditwheel. | Install pinned `patchelf 0.18.0` in the wheel-build venv and prepend that venv to `PATH` for the repair layer. |
